...
class CallRepositoryImpl @Inject constructor(
    ...
    override fun observeCallHistory(): Flow<List<CallRecord>> = flow {
    ...
    override suspend fun initiateCall(userId: String, isVideo: Boolean): String {
        val response = callClient.initiateCall(
            CallOffer(
                calleeId = userId,
    ...
    override suspend fun sendSignal(callId: String, type: String, payload: String) {
    ...
    override fun observeSignals(callId: String): Flow<CallSignal> = callbackFlow {
        ...
        val job = launch {
            callClient.callStream(outgoing).collect { protoSignal ->
                val signal = when (protoSignal) {
                    is ProtoCallSignal.SdpAnswer -> CallSignal(
                        type = "answer",
                        payload = protoSignal.sdp,
                    )
                    is ProtoCallSignal.IceCandidate -> CallSignal(
                        type = "ice-candidate",
                        payload = protoSignal.candidate,
                    )
                    is ProtoCallSignal.Control -> CallSignal(
