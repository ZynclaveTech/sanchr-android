...
class ChatBackupManager @Inject constructor(
    ...
    suspend fun performScheduledBackupIfNeeded(): BackupUploadOutcome? = mutex.withLock {
    ...
    suspend fun backupNow(): BackupUploadOutcome? = mutex.withLock {
        val configuration = recoveryKeyManager.loadConfiguration()
            ?: error("Backups are not enabled on this device.")
        val recoveryKey = recoveryKeyManager.readRecoveryKey()
            ?: error("Recovery key is unavailable.")
        val material = deriveMaterial(recoveryKey)
        performBackup(configuration, material, force = true)
    }

    suspend fun restoreLatestBackup(recoveryKeyOverride: String?): BackupRestoreOutcome = mutex.withLock {
        val recoveryKey = recoveryKeyOverride?.trim()?.takeIf { it.isNotEmpty() }
        ...
        val selected = selectLatestBackup(backups, configuration?.lineageId)
            ?: error("No encrypted backup is available for this account.")

        val download = backupServiceClient.getBackupDownload(
        ...
        val metadata = BackupArchive.json.decodeFromString(
    ...
    suspend fun deleteRemoteBackups() = mutex.withLock {
        val lineageId = recoveryKeyManager.loadConfiguration()?.lineageId
        val backups = backupServiceClient.listBackups(ListBackupsRequest).backups.filter { backup ->
    ...
    private suspend fun performBackup(
        ...
        val metadataBytes = BackupArchive.json.encodeToString(
            BackupOpaqueMetadata.serializer(),
            metadata,
        ).encodeToByteArray()

        val create = backupServiceClient.createBackupUpload(
        ...
        val committed = backupServiceClient.commitBackup(
            CommitBackupRequest(
    ...
    private suspend fun exportSnapshot(): BackupArchiveSnapshot {
        val contacts = contactDao.getAllContacts().map { contact ->
        ...
        val conversations = conversationDao.getAllConversations().map { conversation ->
    ...
    private suspend fun restoreSnapshot(snapshot: BackupArchiveSnapshot) {
        database.withTransaction {
            ...
            val restoredContacts = snapshot.contacts.map { contact ->
                ContactEntity(
                    id = contact.id,
                    userId = contact.userId,
                    phoneNumber = contact.phoneNumber,
            ...
            snapshot.conversations.forEach { conversation ->
                conversationDao.insertConversation(
                    ConversationEntity(
                        ...
                        title = conversation.title,
                        avatarUrl = conversation.avatarURL,
                        participantIds = BackupArchive.json.encodeToString(
    ...
    private fun previewTextFor(contentType: String, contentBody: String): String = when (contentType.lowercase()) {
        "text" -> contentBody
        "image" -> runCatching {
    ...
    private fun selectLatestBackup(backups: List<BackupMetadata>, preferredLineageId: String?): BackupMetadata? {
        val filtered = backups.filter { backup ->
    ...
    private fun httpUpload(url: String, bytes: ByteArray) {
    ...
    private fun hmac(iv: ByteArray, ciphertext: ByteArray, key: ByteArray): ByteArray {
