...
interface RecoveryKeyManager {
    ...
    fun enableBackups(
        recoveryKey: String,
        lineageId: String = UUID.randomUUID().toString().lowercase(),
    ): BackupConfiguration

    fun disableBackups()

    fun updateBackupState(
...
@Singleton
class SignalRecoveryKeyManager
    @Inject
    constructor(
        ...
        override fun updateBackupState(
            timestampMillis: Long?,
            contentHash: String?,
        ) {
