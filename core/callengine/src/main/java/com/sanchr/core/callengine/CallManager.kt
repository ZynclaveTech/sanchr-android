...
class CallManager @Inject constructor(
    ...
    suspend fun startCall(
        ...
        try {
            ...
            val turnCreds = callServiceClient.getTurnCredentials(
            ...
            val response: CallResponse = callServiceClient.initiateCall(
                CallOffer(
                    callId = callId,
    ...
    private suspend fun handleSignalingMessage(callId: String, signal: CallSignal) {
        when (signal) {
    ...
    private fun handleCallEnded(callId: String, reason: CallState.EndReason) {
        durationJob?.cancel()
        durationJob = null
        signalingJob?.cancel()
        signalingJob = null
    ...
    private fun buildIceServers(creds: TurnCredentials): List<PeerConnection.IceServer> {
        ...
        if (creds.urls.isNotEmpty()) {
            servers.add(
                PeerConnection.IceServer.builder(creds.urls)
    ...
    private fun extractCallId(state: CallState): String? = when (state) {
...
sealed interface CallManagerEvent {
    ...
    data class CallConnected(val callId: String) : CallManagerEvent
