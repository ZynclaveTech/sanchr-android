...
sealed class WebRTCEvent {
    ...
    data class RemoteVideoTrackReceived(
        val track: VideoTrack,
    ) : WebRTCEvent()

    data class SignalingStateChanged(
        val state: PeerConnection.SignalingState,
    ) : WebRTCEvent()

    data object IceGatheringComplete : WebRTCEvent()

    data class PeerConnectionError(
        val message: String,
    ) : WebRTCEvent()
}
...
class WebRTCClient
    ...
    constructor(
        ...
        private fun startVideoCapture() {
            ...
            localVideoTrack =
                factory.createVideoTrack(LOCAL_VIDEO_TRACK_ID, videoSource).apply {
                    setEnabled(true)
                }
